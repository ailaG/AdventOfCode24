...
function checkReport(report, failed_limit=2) {
	...
	while (report.length >= 2) {
		...
		if (success) {
			...
			//const rest_of_report = report.slice(3);
			//if (checkStep(level3, level1, prev_sign)) {
			/*if (checkReport([level1, level3, ...rest_of_report])) {
			...
			} else if (checkReport([level2, level3, ...rest_of_report])) {
